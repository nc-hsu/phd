...
- **NEVER touch git unless explicitly told to.** No `git add`, no `git commit`,
  no `git push`, no branch or tag operations — on your own initiative, ever, not
  even for files you just changed at the user's request. The user decides what
  enters the index and what becomes a commit. Writing and updating files in the
  working tree is fine and does not need permission; putting anything into git
  does.
- Note the word "stage" is overloaded here. Writing a file into the working tree
  (which is allowed) is unrelated to `git add` (which is not). The remote-devices
  tooling also uses "stage" for copying a file off the user's machine into the
  session — also unrelated, also fine.
- Before editing a file that was copied off the user's machine earlier in the
  session, FETCH IT AGAIN first. Those copies are point-in-time snapshots and
  the user may have edited the file since. Working from a stale snapshot has
  already clobbered an edit once.
