...
**ICC — intraclass correlation coefficient.** In A1b,

$$
\text{ICC}=\frac{\tau_d^2}{\tau_d^2+\tau_u^2}
$$

Two readings of the same number:

- *Variance share* — the fraction of **true** heterogeneity sitting at the design-group
  level rather than the row level. 0.3 ⇒ 30% of the spread is designs differing from
  each other, 70% is rows within a design differing from each other.
- *Correlation* (hence the name) — for two rows sharing a design group,
  $\operatorname{Corr}(\alpha_i,\alpha_{i'})=\text{ICC}$. It answers "if one building of
  this design ran 5% high, how much does that tell me about another building of the
  same design at a different site?" Near 1 ⇒ almost everything; near 0 ⇒ the design
  label is uninformative.

Note the ratio is in $\tau$'s, not $v_i$: it is a correlation of *true* effects, with
measurement error excluded. Rows also correlate through $v^b_j$, but that is sampling
error, not ICC.

**Two caveats.** The A1b ICC is contaminated by the shared-$b_j$ covariance (same as
$\hat\tau_d^2$ itself) and so reads too high; and its denominator still bundles site
effects with interaction inside $\tau_u^2$. Treat it as a descriptive stepping stone —
the actual claim is carried by $\tau_d$ vs $\tau_s$ in A1c.

*Ref: G&H Ch. 12.2, ~pp. 252–254, where it appears as the pooling factor governing how
far each group's estimate is shrunk toward the grand mean.*
