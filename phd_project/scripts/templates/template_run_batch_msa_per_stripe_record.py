...
## launcher / coordinator (run mode 3): run an MSA parallelised across EVERY
## (stripe, record) pair at once, for maximum concurrency. Uses the same worker as
## run_batch_msa_per_record.py; the only difference is that all stripes' records
## are dispatched together rather than one stripe at a time.
##
...
def run(config_data: str | Path,
    ...
    # build the full flattened list of "{stripe}:{record}" tags up front
    tags = []
    for stripe_idx, stripe_pickle in enumerate(stripe_pickles, start=1):
        ...
        tags.extend(f"{stripe_idx}:{record_tag}" for record_tag in record_tags)
    ...
    ## COLLATE per-record logs -> per-stripe logs -> msa_log.json
    for stripe_idx, stripe_pickle in enumerate(stripe_pickles, start=1):
...
if __name__ == "__main__":
    ...
    #   python run_batch_msa_per_stripe_record.py config_msa.py --max-workers 4
