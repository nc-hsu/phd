...
Produces three figures:
  Figure 1 – Stacked horizontal bar chart: source contributions per site
             (one panel per seismicity class, at a chosen PoE)
"""
...
# ── Colour palette (colour-blind friendly, works in B&W with hatches) ─────────
SOURCE_COLS = {
...
# ─────────────────────────────────────────────────────────────────────────────
# Figure 1 – Source contributions per site — one figure per PoE level
# ─────────────────────────────────────────────────────────────────────────────

def _draw_by_site_panel(ax, rows, seis_label):
    """Draw a single horizontal stacked-bar panel onto *ax* for one seismicity class."""
    ...
    ax.set_xlim(0, 105)
    ax.set_title(f"Seismicity: {seis_label}", fontweight="bold")
    ax.xaxis.set_major_formatter(mpl.ticker.FormatStrFormatter("%g%%"))

    # Dashed lines + region labels at region boundaries
    prev_reg = None
...
def fig_by_site(df: pd.DataFrame,
                fmt: str = "png", dpi: int = 200, out_dir: Path = Path(".")):
    """
    Produce one figure per PoE level, each containing one panel per
    seismicity class, saved as:
        fig1_by_site_poe1_50yr.png
        fig1_by_site_poe2_475yr.png
        ...
    """
    poes = sorted(df["poe"].unique())

    for poe_idx, poe in enumerate(poes, start=1):
        ...
        # Height scales with number of sites per panel
        sites_per_panel = len(sub) // n_panels
        fig_h = max(4, sites_per_panel * 0.38 + 1.5)

        fig, axes = plt.subplots(
            1, n_panels,
            figsize=(7 * n_panels, fig_h),
            sharey=False,
        )
        if n_panels == 1:
            axes = [axes]

        rp_str = POE_RP.get(poe, f"{poe:.5f}")
        fig.suptitle(
            f"Seismic source contributions by site  —  "
            f"PoE {poe}  ({rp_str} return period)",
            fontsize=11, fontweight="bold", y=1.01,
        )

        for ax, seis in zip(axes, seismicity_classes):
            rows = sub[sub["seismicity"] == seis].reset_index(drop=True)
            _draw_by_site_panel(ax, rows, seis)
        ...
        # Build a clean filename: e.g. fig1_by_site_poe2_475yr.png
        rp_slug = rp_str.replace(" ", "").replace(" ", "")
        out = out_dir / f"fig1_by_site_poe{poe_idx}_{rp_slug}.{fmt}"
...
def main():
    ...
    print("\nFigure 1 — source contributions by site (one figure per PoE)")
    fig_by_site(df, fmt=args.fmt, dpi=args.dpi, out_dir=out_dir)
